...
exports.listNewProjectByUserId = asyncErrorHandler(async (req, res, next) => {
  ...
  var projectDetails = await UserProjects.findOne({
    user: userId,
    portfolio: portfolioId,
  });
  if (!projectDetails) {
    projectDetails = new UserProjects({
      user: userId,
      portfolio: portfolioId,
      projects: [],
    });
...
exports.editProjectByUserId = asyncErrorHandler(async (req, res, next) => {
  ...
  const projectDetails = await UserProjects.findOne({
    user: userId,
    portfolio: portfolioId,
  });
  ...
  const projectIndex = projectDetails.projects.findIndex(
  ...
  if (isImageEdited && projectDetails.projects[projectIndex].image.public_id) {
    await cloudinary.uploader.destroy(
      projectDetails.projects[projectIndex].image.public_id
    );
  }
...
exports.deleteProjectByUserId = asyncErrorHandler(async (req, res, next) => {
  ...
  if (
    projectDetails.projects[projectIndex].image &&
    projectDetails.projects[projectIndex].image.public_id
  ) {
    await cloudinary.uploader.destroy(
      projectDetails.projects[projectIndex].image.public_id
